...
def play_button_press_sound():
    pygame.mixer.Sound("audio_files/button_press.mp3").play()

def draw_bets(screen, font, bets):
    bets_text = " | ".join(f"{number}: ${amount}" for number, amount in bets.items())
    bets_display = font.render(f"Bets: {bets_text}", True, (255, 255, 255))
    screen.blit(bets_display, (screen.get_width() // 2 - bets_display.get_width() // 2, 10))
...
def main():
    ...
    roulette_table = pygame.image.load('rooms/r_table.png')
    ...
    belt_image = BeltImage('rooms/belt.png', 0.13)
    ...
    pygame.mixer.init()
    exit_button = ExitButton(screen, "buttons/exit.png", (0, 0))
    ...
    while running:
        ...
        draw_bets(screen, font, bet_buttons.bets)
        ...
        if not spinning and winning_number is not None:
            ...
            if win:
                ...
                # Sprawdzenie czy upłynęła sekunda po wyświetleniu wyniku
                if show_result_time and pygame.time.get_ticks() - show_result_time > 1000:
                    subprocess.call([python_path, "class_fireworks_r.py"])
                    show_result_time = None
